...
bool BNO055I2CActivity::spinOnce() {
    ...
    msg_euler.header.stamp = time;
    msg_euler.header.frame_id = param_frame_id;
    msg_euler.header.seq = seq;
    ...
    // Only publish if record.system_status is good!
    if(record.system_status == 5){
...
bool BNO055I2CActivity::stop() {
    ...
    // Leigh Oliver 24/11/21
    if(pub_euler) pub_euler.shutdown();
